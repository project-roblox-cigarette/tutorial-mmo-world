...
**Dev Container** を使用することで、全員が同じ開発環境で作業できます。

### 初回セットアップ

1. **Docker Desktop** と **VS Code** をインストール
2. VS Code 拡張機能 **Dev Containers** をインストール
3. プロジェクトを VS Code で開く
4. 「Reopen in Container」を選択
5. 初期化スクリプトが自動実行されるのを待つ

### 日常の開発

```bash
# Dev Container 内で
pnpm dev:game  # Rojo サーバー起動

# ホストマシンの Roblox Studio で
# Rojo プラグイン → Connect (ポート 34872)
```

### Dev Container の利点

- ✅ **環境の統一**: 全員が同じ Node.js、pnpm、ツールバージョンで開発
- ✅ **セットアップの簡略化**: Docker があれば即座に開発開始
- ✅ **依存関係の分離**: ローカル環境を汚さない
- ✅ **コンフリクト防止**: 環境差異によるトラブルを排除
